...
#if CONFIG_IDF_TARGET_ESP32S2 // ESP32/PICO-D4
...
void spiAttachMISO(spi_t * spi, int8_t miso)
{
    ...
    char* thingToPrint = (char*) malloc(strlen("New MISO: ") * sizeof(char) + sizeof(int) + 10);
    sprintf(thingToPrint, "New MISO: %d", (int) miso);
...
spi_t * spiStartBus(uint8_t spi_num, uint32_t clockDiv, uint8_t dataMode, uint8_t bitOrder)
{
    if(spi_num >= SPI_COUNT){
        char* thingToPrint = (char*) malloc(strlen("Count too high! It was supposed to be  , but it was actually  !") * sizeof(char) + 2 * sizeof(int) + 10);
        int usrInt = (int) spi_num;
        int spiCount = (int) SPI_COUNT;
        sprintf(thingToPrint, "Count too high! It was supposed to be %d, but it was actually %d!", spiCount, usrInt);
        Serial.println(thingToPrint);
        free(thingToPrint);
        return NULL;
    }
    ...
    if(spi->dev != nullptr)
    {
        Serial.printf("here, dev is working just fine, and in fact here's it's address: %p. And even more so here's a value from the struct: %d\n", (void*) spi->dev, spi->dev->clock.clkcnt_h);
    }
    ...
    if(spi->lock == NULL){
        ...
        if(spi->lock == NULL) {
            Serial.println("lock failed to be made!");
            return NULL;
        }
    }
#endif
    ...
    while(calN <= 0x3F) {
        ...
        while(calPreVari++ <= 1) {
            ...
            if(calFreq == (int32_t) freq) {
                ...
                if(std::abs(static_cast<int64_t>(freq) - static_cast<int64_t>(calFreq)) < std::abs(static_cast<int64_t>(freq) - static_cast<int64_t>(bestFreq))) {
                    bestFreq = calFreq;
                    memcpy(&bestReg, &reg, sizeof(bestReg));
                }
            }
