...
{
    char* thingToPrint = (char*) malloc(strlen("What's passed in - Sck:  , miso:  , mosi:  , ss:  .") * sizeof(char) + 4 * sizeof(int) + 10);
    int firstSck = (int) sck;
    sprintf(thingToPrint, "What's passed in - Sck: %d, miso: %d, mosi: %d, ss: %d.", firstSck, miso, mosi, ss);
    ...
    if(!_spi) {
        Serial.println("This is happening even though we assigned it??");
        return;
    }
    ...
    if(_sck != -1 && _miso != -1 && _mosi != -1 && _ss != -1)
    {
        thingToPrint = (char*) malloc(strlen("All pins ready! Sck:  , miso:  , mosi:  , ss:  .") * sizeof(char) + 3 * sizeof(int));
        sprintf(thingToPrint, "All pins ready! Sck: %d, miso: %d, mosi: %d, ss: %d.", _sck, _miso, _mosi, _ss);
        Serial.println(thingToPrint);
        free(thingToPrint);
    }
}
